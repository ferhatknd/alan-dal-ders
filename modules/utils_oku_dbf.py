...
48.4% başarı oranıyla çalışan, test edilmiş fonksiyonlar.

KAYNAK: extract_olcme.py (2025-07-27)
BAŞARI ORANI: 48.4% (585/1164 perfect matches)
YAKLAŞIM: Simple String Matching + Pattern Matching + PyMuPDF Unified Processing
"""
...
def ex_kazanim_tablosu(full_text):
    """
    extract_olcme.py'den kopyalandi - PDF'den KAZANIM SAYISI VE SÜRE TABLOSU'nu çıkarır ve formatlı string ile yapılandırılmış veri döndürür
    
    Args:
        full_text (str): PDF/DOCX'den çıkarılan tam metin
        
    Returns:
        tuple: (formatlı_string, yapılandırılmış_veri_listesi)
    """
    try:
...
def extract_ob_tablosu(full_text):
    ...
    Returns:
        str: Öğrenme birimi analiz sonucu
    """
    try:
