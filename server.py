...
def update_nlp_statistics(operation_type, input1, input2, success):
    """NLP işlem istatistiklerini günceller"""
    if operation_type == 'semantic_similarity':
        nlp_stats['semantic_similarity']['total_requests'] += 1
        nlp_stats['semantic_similarity']['total_query_chars'] += len(input1)
        nlp_stats['semantic_similarity']['total_content_chars'] += len(input2)
        if success:
            nlp_stats['semantic_similarity']['successful_matches'] += 1
        else:
            nlp_stats['semantic_similarity']['failed_matches'] += 1
    
    elif operation_type == 'bert_correction':
        nlp_stats['bert_correction']['total_requests'] += 1
        nlp_stats['bert_correction']['total_original_chars'] += len(input1)
        nlp_stats['bert_correction']['total_corrected_chars'] += len(input2)
        if success:  # success means text was changed
            nlp_stats['bert_correction']['texts_changed'] += 1
        else:
            nlp_stats['bert_correction']['texts_unchanged'] += 1

app = Flask(__name__)
CORS(app)

@app.route('/api/nlp/statistics')
def get_nlp_statistics():
    """NLP işlemleri için özel istatistikleri döndürür"""
    return jsonify({
        'nlp_stats': nlp_stats,
        'semantic_similarity': {
            'total_requests': nlp_stats['semantic_similarity']['total_requests'],
            'success_rate': round(
                (nlp_stats['semantic_similarity']['successful_matches'] / 
                 max(nlp_stats['semantic_similarity']['total_requests'], 1)) * 100, 2
            ),
            'avg_query_length': round(
                nlp_stats['semantic_similarity']['total_query_chars'] / 
                max(nlp_stats['semantic_similarity']['total_requests'], 1), 1
            ),
            'avg_content_length': round(
                nlp_stats['semantic_similarity']['total_content_chars'] / 
                max(nlp_stats['semantic_similarity']['total_requests'], 1), 1
            )
        },
        'bert_correction': {
            'total_requests': nlp_stats['bert_correction']['total_requests'],
            'change_rate': round(
                (nlp_stats['bert_correction']['texts_changed'] / 
                 max(nlp_stats['bert_correction']['total_requests'], 1)) * 100, 2
            ),
            'avg_original_length': round(
                nlp_stats['bert_correction']['total_original_chars'] / 
                max(nlp_stats['bert_correction']['total_requests'], 1), 1
            ),
            'avg_corrected_length': round(
                nlp_stats['bert_correction']['total_corrected_chars'] / 
                max(nlp_stats['bert_correction']['total_requests'], 1), 1
            )
        }
    })

@app.route('/api/nlp/semantic-similarity', methods=['POST'])
def nlp_semantic_similarity():
    ...
    try:
        result_index = semantic_find(query, content)
        
        # NLP istatistiklerini güncelle
        update_nlp_statistics('semantic_similarity', query, content, result_index != -1)
...
@app.route('/api/nlp/bert-correction', methods=['POST'])
def nlp_bert_correction():
    data = request.get_json()
    text = data.get('text')
    ...
    try:
        corrected_text = correct_turkish_text_with_bert(text)
        
        # NLP istatistiklerini güncelle
        update_nlp_statistics('bert_correction', text, corrected_text, text != corrected_text)
